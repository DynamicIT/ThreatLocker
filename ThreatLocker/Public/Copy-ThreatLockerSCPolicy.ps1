function Copy-ThreatLockerSCPolicy {
    ...
    process {
        $sourcePolicy = Get-ThreatLockerSCPolicyDetail -Org $orgId -StoragePolicyId $StoragePolicyId
        if ($NewGroup) {
        ...
        $splat = @{
            Method = 'POST'
            Endpoint = 'StoragePolicy/StoragePolicyInsert'
            OrgId = $newOrgId
            Body = $clone
        }
